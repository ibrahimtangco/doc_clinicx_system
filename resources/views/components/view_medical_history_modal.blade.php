...
<div
	class="view-medical-history-modal hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full bg-black/50 backdrop-blur-sm">
	<div class="relative p-4 w-full max-w-2xl max-h-full">
		...
		<div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
			...
			<div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
				...
				<button
					class="view-medical-history-close-btn text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white"
			...
			<div class="p-4 md:p-5 space-y-4">
				<form class="space-y-2" method="POST">
                    <input type="text" class="hidden" name="id" id="edit-id">
					<div class="w-full">
						<x-input-label :value="__('Condition')" for="condition" />
						<x-text-input disabled value="{{ old('condition') }}" autofocus class="mt-1 w-full" id="edit-condition" name="condition" type="text" />
						<x-input-error :messages="$errors->get('condition')" class="mt-2" />
					</div>
					<div class="w-full">
						<x-input-label :value="__('Diagnosed Date')" for="diagnosed_date" />
						<x-text-input disabled :value="old('diagnosed_date')" autofocus class="mt-1 w-full" id="edit-diagnosed_date" name="diagnosed_date"
							type="date" />
						<x-input-error :messages="$errors->get('diagnosed_date')" class="mt-2" />
					</div>
					<div class="w-full">
						<x-input-label :value="__('Treatment')" for="treatment" />
						<x-text-input disabled :value="old('treatment')" autofocus class="mt-1 w-full" id="edit-treatment" name="treatment" type="text" />
					...
					<div class="w-full">
						...
						<textarea disabled rows="2" class="mt-1 border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm w-full"
						 id="edit-description" name="description"></textarea>
