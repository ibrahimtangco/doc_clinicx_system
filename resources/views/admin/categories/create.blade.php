					...
					<section>
						...
						<form action="{{ route('categories.store') }}" class="mt-6 space-y-6" method="post">
							...
							<div class="mt-4">
								...
								<x-text-input :value="old('name')" autocomplete="name" autofocus class="block mt-1 w-full {{ $errors->has('name') ? 'is-invalid' : ''}}" id="name"
									name="name" type="text" />
							...
							<div class="mt-4">
								...
								<textarea name="description" id="description" rows="3"
									class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm w-full mt-1 {{ $errors->has('description') ? 'is-invalid' : ''}}">{{ old('description') }}</textarea>
