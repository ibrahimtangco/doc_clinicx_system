    ...
    <div class="py-4">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="overflow-hidden">
                <form action="{{ route('user.reserve.store', ['service' => $service]) }}"
                    class="flex flex-col gap-4 max-w-xl" method="POST">
                    @csrf
                    <div class="w-full">
                        ...
                        <select
                            ...
                            @foreach ($availableDates as $availableDate)
                                <option value="{{ $availableDate->date }}">
                                    {{ Carbon\Carbon::parse($availableDate->date)->format('F j, Y') }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="w-full">
                        ...
                        <div id="available-slots" class="text-sm text-gray-500"></div>
                    </div>
                    <div class="w-full">
                ...
                if (selectedDate) {
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            $('#slot-select').empty();
                            $('#slot-select').append('<option value="">Select Schedule</option>');
                            ...
                            // Update available slots when AM or PM is selected
                            $('#slot-select').change(function() {
                                const selectedSlot = $(this).val();
                                if (selectedSlot === 'AM') {
                                    $('#available-slots').html(
                                        `${am_capacity} slots available for AM.`);
                                } else if (selectedSlot === 'PM') {
                                    $('#available-slots').html(
                                        `${pm_capacity} slots available for PM.`);
                                } else {
                                    $('#available-slots').html('');
                                }
                            });
                        ...
                        error: function(status) {
                            console.log(status)
                            alert(
                                'Unable to fetch capacity data for the selected date. Try again later');
                        }
                    });
                } else {
                    $('#slot-select').empty();
