					...
					<tbody>
						@foreach ($userReservations as $reservation)
							<tr class="bg-white border-b hover:bg-gray-50">
								<td class="px-6 py-4">{{ $reservation->patient->user->full_name }}</td>
								<td class="px-6 py-4">{{ $reservation->service->name }}</td>
								<td class="px-6 py-4">{{ Carbon\Carbon::parse($reservation->date)->format('F j, Y') }}</td>
								<td class="px-6 py-4">{{ $reservation->preferred_schedule }}</td>
								<td class="px-6 py-4">{{ $reservation->queue_number ?? '---------------' }}</td>
								@php
									$statusClasses = [
								...
								<td class="px-6 py-4 {{ $statusClasses[$reservation->status] ?? 'text-gray-500' }} capitalize italic font-bold">
									{{ $reservation->status }}
								</td>
