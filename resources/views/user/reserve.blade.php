<x-app-layout>
	...
	<div class="max-w-7xl mx-auto py-4 px-4 sm:px-6 lg:px-8">
		<p class="text-sm font-semibold flex items-center"><a href="{{ route('user.dashboard') }}">Services</a>
			...
			<!DOCTYPE svg PUBLIC '-//W3C//DTD SVG 1.1//EN' 'http://www.w3.org/Graphics/SVG/1.1/DTD/svg11.dtd'><svg
				fill="currentColor" height="18px" id="Layer_1" style="enable-background:new 0 0 512 512;" version="1.1"
				viewBox="0 0 512 512" width="18px" xml:space="preserve" xmlns:xlink="http://www.w3.org/1999/xlink"
		...
		<div class="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8">
								...
								@if (!$appointment['off'])
									@foreach ($appointment['business_hours'] as $time)
										...
										@if (!in_array($time, $appointment['reserved_hours']))
											@if ($time !== '12:00')
												<form action="{{ route('reserve.appointment') }}" id="appointment-form" method="post">
													@csrf
										...
										@else
											<button
												class="w-full px-4 py-2 block bg-gray-400 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest mb-6"
												disabled>{{ $standardTime }}</button>
										@endif
									@endforeach
								@else
									@foreach ($appointment['business_hours'] as $time)
			...
			<div class="mt-8 space-y-4">
				<div class="flex items-center gap-2">
					<button
						class="cursor-text p-4 block bg-primary border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest "></button>
					<span class="font-semibold text-sm">Available </span>
				</div>

				<div class="flex items-center gap-2">
					<button
						class="cursor-text p-4 block bg-gray-400 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest "></button>
	...
	<script src="{{ asset('js/appointment-card.js') }}"></script>
</x-app-layout>
